	...
	public function login(array $data)
	{
		...
		if($result){
			...
			$group = Db::table('ike_admintype')
						...
						->select();
			$group=reset($group);
			if(Config::get('SUPERVISOR')!=$data['admin_name'])
			{
				if($group['status']==0){
		...
		if(empty($result) || $result['status']==0 || $result['password'] !== md5($data['password'])){
			if(empty($result)){
				...
				if($data['admin_name']!=Config::get('SUPERVISOR'))
				{
		...
		Session::set('loginSession',$result,'login');
		return $result;
	}
	/*退出登录*/
	public function logout()
	{
		$admin_id=Session::get('loginSession','login')['admin_id'];
		Db::table('ike_admin')
