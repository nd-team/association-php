	...
	**/
	...
	public function selectRecommendInfo(array $data)
	{
		...
		if($result)
		{
			...
			$datas['fatherName']=$result['fatherName'];
			$datas['motherName']=$result['motherName'];
			if($result['marriage']==1)
			{
				$datas['marriage']=$result['marriage'];
				$datas['spouseName']=$result['spouseName'];
				$datas['childrenName']=$result['childrenName'];
				$datas['childrenSchool']=$result['childrenSchool'];
			}else{
				$datas['marriage']=$result['marriage'];
			}
	...
	*/
		...
		if($data['status']==1)
		{
			if($check_claim)
			{
				foreach($check_claim as $k=>$v)
				{
					...
					$userInfo=$this->userinfo($v['mobile']);
	...
	public function directNexusChart(array $data)
	{
		...
		if($beRecommender)
		{
			...
			$userinfo=$this->userinfo($v['userId']);
			if(!$userinfo)
			{
				continue;
			}
